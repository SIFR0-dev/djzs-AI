...
**v1.6 (2026-09-09, guard-checked: zero origin: pool records exist).** §3's exclusion — a pool candidate is skipped when an existing record already binds it — is read as primary-eligible records only: records with deviated: true exclude nothing, since a deviated record never enters the primary. Exclusion matches at market level; an event ticker excludes only the market a record names, not its siblings. Identifiers are compared within their own space: a Polymarket record's market.ticker is a Gamma slug, the pool ranks on conditionId, and the reader carries whichever id the record holds and compares both.

**v1.7 (2026-09-09, guard-checked: zero origin: pool records and zero graded records exist).** Two parts: what is recorded, and how §6 may use it.

(a) Liquidity at audit. Every record sealed after this amendment carries, alongside price_at_audit, the bound market's traded volume: volume_24h, the traded USD notional in the 24 hours ending at posted_at, and volume_total, the cumulative traded USD notional to posted_at. Both are computed from the same trade data as the VWAP, by the same source tier (Polymarket: the Dune saved query; Kalshi: the venue trades API), and are therefore reproducible by re-execution exactly as the price is; q3-verify re-checks them on the same schedule as the price. Series bindings, which name no venue market, carry null. Records sealed before this amendment carry neither; the fields are optional in the schema and forward-only, and their absence is not a verification failure, because sealed records are immutable.

(b) The thin-market prior — §6. The primary analysis, verdict against resolution, is unchanged and takes no account of liquidity: a resolution is a resolution at any volume. But wherever §6's analysis uses price_at_audit as a probability baseline, comparator, or control, the quote on a low-volume contract may be an echo of the same narrative under audit rather than an independent estimate — a correlated prior presented as an independent one. Any such analysis must therefore (i) report the thin stratum separately and never pool it into a headline figure without also showing the split; (ii) use volume_24h < 25,000 USD at audit as the thin threshold, fixed here before any outcome exists and chosen so that the §3 coverage pool, which selects on 24h volume, has sat entirely above it in observation to date; and (iii) additionally report liquidity as a continuous covariate, so that the threshold's arbitrariness is visible rather than load-bearing.

No selection rule changes. §3's coverage pool is unchanged and scan bindings remain unconstrained by volume: liquidity is recorded and stratified, never filtered. A filter added mid-study would change what the study covers, and coverage was pre-registered.

**v1.8 (2026-09-09, drafted before the filtered pool ranking was observed; guard-checked: zero origin: pool records exist).** §3's coverage pool additionally excludes venue-native recurrence markets: a candidate is out if its tag set contains any of Recurring, Up or Down, or an interval tag (5M, 15M, 1H, 4H), matched case-normalized as whole tags. The ground is a precondition of the study, not a preference among markets: DJZS audits the reasoning stated in a public narrative before an outcome is known, and a contract that opens and resolves inside minutes or hours admits no such narrative — there is no interval in which a thesis can be stated, sourced, and bound before resolution. This is mechanically checkable from the venue's own tags and does not require the excluder to judge a market's content or quality. Scan bindings are unaffected: if a narrator ever states a thesis on such a contract, it may be bound as usual. Sports, esports, entertainment and weather remain excluded per v1.5; nothing else in §3 changes.

**v1.9 (2026-09-09, drafted before the filtered pool ranking was observed; guard-checked: zero origin: pool records exist).** v1.8's recurrence exclusion is restated venue-neutrally so that it means the same thing on every venue. A pool candidate is excluded when the interval between its posted_at and its scheduled resolution is less than 24 hours, computed from the venue's own published close or expiration time. Polymarket's recurrence tags (Recurring, Up or Down, 5M, 15M, 1H, 4H) remain an equivalent operational proxy where the tag set is available and the close time is not, and where both are available the close time governs. The ground is unchanged and remains a precondition of the study rather than a judgement about markets: a contract resolving within a day admits no interval in which a public narrative can be stated, sourced, and bound before the outcome is known. Scan bindings are unaffected. Nothing else in §3 changes.

**v1.10 (2026-09-10, written after the first pool day's composition was observed and before any pool record resolved; guard-checked: zero graded pool records exist).** Every record carries event_key, a stable identifier for the underlying real-world event its market resolves on: the venue's own event ticker where one is published, otherwise an operator-assigned key naming the resolution source and date. §6 treats records sharing an event_key as one cluster and never as independent observations: any statistic reported over records must be computed over clusters, or reported with cluster-robust uncertainty, and in either case must state the number of distinct events alongside the number of records. Where records on one event are mutually exclusive by construction — a strike ladder in which exactly one outcome resolves YES — that fact must be stated wherever their outcomes are reported, because the outcome distribution there is fixed by market structure rather than by anything the audit did. Disclosure: this amendment was written after the 2026-09-10 pool composition was seen (seven of ten records on the September FOMC decision, five of them a mutually exclusive ladder), and before any pool record resolved.

**v1.11 (2026-09-10, guard-checked: zero graded pool records and zero records carrying event_key exist).** v1.10's identifier clause is corrected. event_key is always operator-assigned and identifies the real-world event a market resolves on, independent of venue: it names the resolving authority, the decision or measurement, and its scheduled date, in a form stable across venues and listings (for example, FOMC-2026-09-16). Separately, every record carries venue_event_key, the venue's own published event identifier verbatim where one exists, and null where none does. Both are sealed at Phase A. §6 clusters on event_key and never on venue_event_key; the mutual-exclusivity disclosure v1.10 requires likewise applies across venues, since a strike ladder and another venue's strikes on the same decision are mutually exclusive in substance whatever their listings say. v1.10 said the venue ticker governs where published; that made clustering venue-scoped and would have treated two venues' view of one decision as independent evidence — the defect v1.10 was written to prevent. Nothing else in v1.10 changes.

**v1.12 (2026-09-10, guard-checked: zero pool records exist).** §3's "Every pooled market is audited. None skipped" and the authoring clause requiring intent.thesis be a verbatim sourced public case are both binding, and this amendment states how they meet when a selected market has no dominant public case for either side.

Such a market is never skipped and never deviated. It is audited and sealed as a primary-eligible record with thesis_state: "no_public_case", intent.thesis null, and a search_record naming what was searched — the sources consulted, the queries used, the window, and the timestamp — so the absence is itself evidenced rather than asserted. No thesis may be written, paraphrased, or reconstructed from the market's own question, price, or structure; a market's existence is not a case for it.

Such a record is primary-eligible and enters §6, reported as its own stratum. Its engine verdict is whatever the deterministic engine returns on an input whose thesis is absent, and that verdict is a finding about the market, not a defect in the record. §6 must report the size of this stratum alongside any statistic computed over pool records, because a pool in which many selected markets carry no public case is a different object from one in which few do, and the proportion is itself a result.

The absence must be re-checked at grading: if a dominant public case appeared after posted_at, the record is graded as sealed and the later emergence is noted, never retrofitted into the sealed record.

§9's no-source-no-record rule is scoped to scan narratives seeking a market and does not reach a pooled market seeking a narrative; the two directions are distinct and this amendment governs the second.

**v1.13 (2026-09-14, written after the 2026-09-14 pool composition was observed and before any pool record was sealed; guard-checked: zero pool records exist).** A market that resolves jointly on more than one distinct real-world event — a combination market — cannot be described by a single `event_key` without misstating what it is, and §3 does not permit skipping it. Such a record carries `event_keys`: an array of two or more operator-assigned keys, one per component event, each satisfying v1.11. Its `event_key` is the component keys joined by `+` in lexical order; that compound value is a label, not a cluster. §6 treats the record as a member of every component cluster: it is counted within each, is never independent of any, and is never counted as an observation of its compound label. `venue_event_key` is unchanged. Single-event records carry `event_key` alone as before; an `event_keys` array with fewer than two entries is invalid. Disclosure: written when `KXBALANCEPOWERCOMBO-27FEB-RR` (House control AND Senate control) entered the Kalshi top five on 2026-09-14, before any pool record was sealed.
