...
async function polymarketPool(exclude: Set<string>) {
  ...
  return { rows: rows.sort((x, y) => y.volume_1d - x.volume_1d), events_scanned: events, dropped_by_book: dropped, page_cap: 1000, capped: events >= 1000 };
}
(async () => {
