...
export async function runDuneQuery(queryId: number, params: Record<string, string | number>, opts: { timeoutMs?: number; fetchImpl?: typeof fetch } = {}): Promise<DuneRun> {
  ...
  // 2026-09-09: Dune began rejecting an explicit "medium" ("Invalid performance tier"); omit so the account default applies, override via DUNE_PERFORMANCE if ever needed
  const ex = await f(`${BASE}/query/${queryId}/execute`, { method: "POST", headers: H, body: JSON.stringify({ query_parameters: params, ...(process.env.DUNE_PERFORMANCE ? { performance: process.env.DUNE_PERFORMANCE } : {}) }) });
