...
**Open question for the operator, surfaced before any record exists.** Kalshi rank 5, `KXBALANCEPOWERCOMBO-27FEB-RR` ("House Control Republican **AND** Senate Control Republican"), is a conjunction. §1's committed combo rule excludes multivariate markets operationally, by `mve_collection_ticker` or a non-`binary` `market_type`, and this market carries neither, so the code admits it correctly. Whether a semantically conjunctive market belongs in the pool is a judgement the operational test does not make. It sits in today's top five, so it is worth ruling on before pool records are created.
