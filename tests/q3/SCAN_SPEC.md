...
## 1. §3 coverage pool — category implementation (v1.5 rules 1–2, v1.8, v1.9)
...
|---|---|---|---|---|
| Polymarket | `polymarket_polygon.market_details.tags` (Dune) = Gamma event tags, 3–7 per market | `Politics` · `Elections` · `Geopolitics` · `World` · `Economy` · `Fed` · `Finance` · `Crypto` | v1.5, always: `Sports` · `Esports` · `Culture` · `entertainment` · `Weather` — v1.8, **fallback proxy only** (see the v1.9 row): `Recurring` · `Up or Down` · `5M` · `15M` · `1H` · `4H` | `lib.ts` (`POOL_TAGS_INCLUDE` / `POOL_TAGS_EXCLUDE_CATEGORY` / `POOL_TAGS_EXCLUDE_RECURRENCE` + `poolCategoryAdmit`, the single definition) · `queries/polymarket_pool.sql` header (same strings, same matching rule) · `tape/discover.ts` and `dune-publish.ts` (both call the shared matcher) · `pool-matcher.test.ts` (runs on every push) |
...
| **both** (v1.9 duration) | Kalshi `market.close_time` · Polymarket `market_details.market_end_time` `timestamp(3) with time zone` (Dune) / `market.endDate` (Gamma) | interval from the read to the published close **≥ 24 h** | anything closing in **< 24 h**, and anything already past its close | `lib.ts` (`POOL_MIN_HOURS_TO_CLOSE`, `hoursToClose`, `poolDurationAdmit`) · `queries/polymarket_pool.sql` (`classified`) · `tape/discover.ts` (both venue reads) · `dune-publish.ts` (re-checked on returned rows) · `pool-matcher.test.ts` |

Mapping decisions, recorded:
- **Matching rule (Polymarket): array containment on whole tags, both sides lower-cased.** Never a substring or word-boundary regex. The tag set is non-positional, mixed case, 3–7 per market, and carries non-ASCII, and v1.8's exclusions include the bare interval tags `5M` `15M` `1H` `4H` — a boundary regex fires on those inside unrelated text, and the live vocabulary carries an unrelated `Finance Updown` that a substring match would wrongly catch, while containment matches only the whole tag. Tags reach the tooling three ways (a real array from Gamma, a JSON-array string, a comma-delimited string from Dune) and `normalizeTags` folds all three to lower-cased whole tags, so the match holds under any stored form. A market carrying any EXCLUDE tag is out even if it also carries an INCLUDE tag: a sports market tagged `Politics` is still sports.
...
- **v1.8 — venue-native recurrence is out of the pool.** `Recurring`, the single tag `Up or Down`, and the interval tags `5M` `15M` `1H` `4H` reject a candidate. The ground is a precondition of the study rather than a preference: a contract that opens and resolves inside minutes admits no public narrative to audit before the outcome is known. It is checkable from the venue's own tags, so the excluder never judges a market's content. Scan bindings are unaffected — a narrator who does state a thesis on such a contract may still bind it.
- **v1.9 — the duration rule, and how it relates to v1.8.** A candidate is out when the interval from the read to its scheduled resolution is under 24 hours, computed from the venue's own published close or expiration time. **Where a close time exists it governs**, in both directions: a Polymarket market tagged `1H` that closes a week out is admitted, and an untagged market that closes in an hour is excluded. The v1.8 recurrence tags remain in force **only as the fallback proxy**, for candidates whose close time the venue does not publish (or publishes unparseably). The v1.5 category exclusions are untouched and apply either way. The threshold is a hard `≥ 24 h`: exactly 24 hours is admitted, since the amendment excludes *less than* 24 hours. A close time already in the past is a negative interval and is therefore excluded.
- **v1.9 closed the Kalshi gap this section previously named.** v1.8 was written against a tag set and Kalshi publishes categories, not tags, so its hourly and 15-minute strike ladders stayed poolable while their Polymarket equivalents were excluded. v1.9's venue-neutral restatement reaches them through `close_time`, which is the venue's own number and needs no ticker-pattern matching — so the gap is closed without the content judgement that closing it by ticker would have required. Measured on the 2026-09-10 venue-direct read: the rule dropped **7,009** Kalshi markets and **202** Polymarket markets; neither venue's top 5 by 24h volume changed, which is the expected shape — short-dated ladders are numerous, not large.
- **`market_end_time` granularity: it is a true `timestamp(3) with time zone`, so a midnight value is a midnight *close*.** Roughly 43% of open markets close at exactly `00:00:00Z` (measured on the same underlying field via Gamma, 2026-09-10: 597 of 1,395). That is a venue scheduling convention, not a date that lost its clock. **The boundary is handled by taking the published value exactly as-is and never rounding it to end-of-day** — the same ruling as before, but it now rests on the column's type rather than on an inference about serialization.
- **Correction, recorded rather than quietly edited.** An earlier revision of this section read the midnight cluster as a *date serialized with a zero time* and concluded that the duration test carried a one-directional bias: intervals understated by up to 24 h, so the rule might exclude a market with 24–48 h left. **That is withdrawn.** It was an artifact of treating the column as a VARCHAR that had to be parsed; against the real type there is no serialization step, no rounding error, and **no directional bias** — the rule compares the venue's own instant to `now()` and nothing is inferred. The safe-direction argument is not needed because there is no error to be safe about.
- **The recurrence oracle, and why it is an oracle and not a criterion.** A venue-native recurrence market names itself in its own URL (`updown-<n>m` / `updown-<n>h` in `market_details.polymarket_link`). The pool query **never filters on that** — nothing in either SQL file matches a slug, and v1.9's rule is the published close time alone. The slug is used only in the opposite direction, as independent ground truth *about* the rule: at republish, `dune-publish` asserts that every market whose link matches `updown-\d+[mh]` was excluded by the duration test. Using the slug as the criterion would make pool membership a naming judgement; using it as the oracle checks the mechanical rule against a fact the rule never saw. **The assertion carries a non-vacuity guard**: if the oracle pattern matches no row at all it reports that it could not be exercised, rather than passing. A check that silently matches nothing is green forever and proves nothing — the same failure class as a publish check that never exercises its own NULL branch.
- **A candidate with no usable close time is admitted and counted, never dropped silently.** On Polymarket the v1.8 tags still decide; on Kalshi there is no tag set to fall back to, so the market is admitted and recorded under `no_close_time` in the day's JSON and in the run banner. An unparseable value parses to `null`, never to a date: excluding or admitting a market on an interval nobody published would be a fabricated number, and rule 2 forbids asserting what the venue did not say.
- **No defensive parse or cast survives on a correctly-typed column, in either query.** The rule the last three failures shared was not three separate bugs but one habit: coercing a column that was already the right type, which converts a loud type error into either a silent wrong answer or a query that cannot compile. Both files now carry an explicit inventory of every coercion they still contain, with the two differing types each one bridges; `market_end_time` is compared to `now()` directly, and `tags` is consumed as the array it is. `polymarket_price.sql` contained no member of the class — and unlike the pool query it has executed live many times with its VWAP verified against sealed records, so its coercions are empirically correct rather than merely argued.
- **The pool query returns `close_time` and `close_basis` for the same reason it returns `tags`** — so a re-run shows *which* test admitted each row. If every returned row comes back `close_basis: "v1.8 tag proxy"`, `market_details.market_end_time` is not populated and the duration rule is a no-op on Polymarket; `dune-publish` prints that fact rather than letting the rule silently fall back.
...
- **The matcher has tests, and they run on every push.** `tests/q3/pool-matcher.test.ts` (49 assertions, no network, no key) pins the tag vocabulary, `normalizeTags`' three input shapes, the v1.8 behaviour as a regression set, `hoursToClose`, and every branch of v1.9 — the 24 h boundary in both directions, a past close, a close time overriding a recurrence tag, a recurrence tag standing in where no close time exists, and an unparseable value falling through to the proxy. It is a step in the `q3-integrity` gate job.
...
`npx tsx tests/q3/dune-publish.ts --update` PATCHes the committed SQL onto 8601185, verifies the published text equals the file, and runs the contract checks: five rows, nine columns (`tags` added in v1.5, `close_time` and `close_basis` in v1.9), decimal token ids, the exclude path, **every row's `tags` satisfies §1**, and — new — **every row satisfies v1.9**, re-checked through the same shared matcher the venue-direct read uses, so the query and the tooling cannot disagree about coverage. It also reports how many rows were admitted by a published close time versus the v1.8 proxy, so a `market_end_time` that is never populated shows up as a stated fact instead of a silent fallback. Needs `DUNE_API_KEY` (Analyst plan). The first republish after the v1.5 SQL also confirms the stored format of `market_details.tags` (documented by Dune only as "Market category tags from the API"); the whole-word match was written to hold under either format, and the check is what proves it.
