...
| **both** (v1.9 duration) | Kalshi `market.close_time` · Polymarket `market_details.market_end_time` (Dune) / `market.endDate` (Gamma) | interval from the read to the published close **≥ 24 h** | anything closing in **< 24 h**, and anything already past its close | `lib.ts` (`POOL_MIN_HOURS_TO_CLOSE`, `hoursToClose`, `poolDurationAdmit`) · `queries/polymarket_pool.sql` (`classified`) · `tape/discover.ts` (both venue reads) · `dune-publish.ts` (re-checked on returned rows) · `pool-matcher.test.ts` |
